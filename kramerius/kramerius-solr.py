...
for K in KRAMERIUS:
	...
	for I in ISSN:
		...
		# ISSN ROOT
		ROOT=[]
		try: req = session.get(K + 'api/v5.0/search?q=issn:' + escape(I) + '&fl=root_pid&rows=1000000&wt=json')
		except:
			sys.stderr.write('Session error: ' + I + ' -> ' + K + '\n')
			continue
		if req.status_code == 200:
			try:
				resp = json.loads(req.text)
			except:
				sys.stderr.write('JSON error: ' + I + ' -> ' + K + '\n')
				continue
		...
		for R in ROOT:
			...
			try: req = session.get(K + 'api/v5.0/search?q=root_pid:' + escape(R) + ' AND document_type:periodicalitem' + '&fl=PID,title,dc.title,details&rows=1000000&wt=json')
			except:
				sys.stderr.write('Session error: ' + I + ' -> ' + K + ' -> ' + R + '\n')
				continue
			FN = str(uuid.uuid4())
			try:
				...
				continue
			# ISSN ROOT PAGE
			try: req = session.get(K + 'api/v5.0/search?q=root_pid:' + escape(R) + ' AND document_type:page&fl=PID,parent_pid,rok,title,dc.title&rows=1000000&wt=json')
			except:
				sys.stderr.write('Session error: ' + I + ' -> ' + K + ' -> ' + R + '\n')
				continue
			FN = str(uuid.uuid4())
