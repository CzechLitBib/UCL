...
if (!empty($_POST)) {
	...
	if (!empty($_POST['email-username']) and !empty($_POST['email-passwd']) and !empty($_POST['email-server'])) {
		if (isset($_POST['email-save'])) {
			...
			if (!$query) {
				$_SESSION['result'] = "Zápis odesílatele selhal.";
			} else {
				$_SESSION['result'] = "Odesílatel uložen."; 
			}
		}
	}
...
if ($db) {
	...
	while ($result = $query->fetchArray(SQLITE3_ASSOC)) {
		...
		$email_passwd = $result['passwd'];
		$email_server = $result['server'];
	}
}
...
<table class="table my-4">
	...
	<tbody>
	<tr>
	<td class="col align-middle"><input type="email" class="form-control" id="email-username" name="email-username" value="<?php echo $email_username; ?>"></td>
	<td class="col align-middle"><input type="text" class="form-control" id="email-passwd" name="email-passwd" value="<?php echo $email_passwd; ?>"></td>
	<td class="col align-middle"><input type="text" class="form-control" id="email-server" name="email-server" value="<?php echo $email_server; ?>"></td>
	<td class="align-middle text-center">
		<div class="row-3">
		<input type="submit" id="email-save" name="email-save" value="email-save" hidden>
		<svg xmlns="http://www.w3.org/2000/svg" onclick="email_on_save()" width="24" height="24" fill="currentColor" class="bi bi-arrow-down-square" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M15 2a1 1 0 0 0-1-1H2a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V2zM0 2a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V2zm8.5 2.5a.5.5 0 0 0-1 0v5.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V4.5z"/></svg>	
		</div>
	</td>
	</tr>
</tbody>
