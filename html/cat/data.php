...
<nav class="navbar container-fluid navbar-expand-md navbar-dark" style="background-color:#dc3545;">
	<div class="row align-items-center gx-0">
		...
		<div class="col"><a class="navbar-brand nav-link active" href="/cat/">Vývoj # CAT # <?php if (isset($_GET['sif'])) { echo $_GET['sif']; } ?></a></div>
	</div>
...
if (isset($_GET['sif']) and !empty($data)) {
	if (array_key_exists($_GET['sif'],$data)) {
		$ids = $data[$_GET['sif']]['ident'];
		rsort($ids);
		...
		foreach ($ids as $ident) {
			...
			. '<a class="external-link" target="_blank" href="https://aleph.lib.cas.cz/F/?func=direct&doc_number='
			. $ident . '&local_base=AV&format=001">'
			. $ident . '</a>'
			. '</div>';
		}
...
if (!empty($error)) {
	echo '<div class="col-8"><div class="alert alert-warning alert-dismissible fade show" role="alert">' . $error . '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div></div>';
}
