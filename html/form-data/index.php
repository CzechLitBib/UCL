	...
	if ($error) {
		...
		if (!$data->fetchArray()) {
			...
			while ($row = $data->fetchArray()) {
				...
				. '<svg xmlns="http://www.w3.org/2000/svg" onclick="card_toggle(' . "'" .   $row[0] . "'" . ')" width="24" height="24" fill="currentColor" class="bi bi-justify" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M2 12.5a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 1-.5-.5zm0-3a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 1-.5-.5zm0-3a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 1-.5-.5zm0-3a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 1-.5-.5z"/></svg></td>';# CARD
				echo '<td class="align-middle text-nowrap">' . date(" d.m Y", hexdec(substr($row[0],0,8))) . '</td>'# ID
				. '<td class="align-middle text-start col-6">' . $row[1] . '</td>';# FORMAT

				echo '<td class="text-end"><button type="button" class="btn btn-secondary btn-sm" onclick="on_done()">Zpracováno</button></td>';
			
			echo '</tr>';
			echo '<tr class="collapse" id="collapse-' . $row[0] . '"><td colspan="4">';
				echo '<table class="table table-light table-borderless m-0 my-4"><tbody>';
					...
					. 'Odkaz'
					. '</b></td><td class="text-start align-middle">';
					echo '<a class="external-link" href="https://clb.ucl.cas.cz" target="_blank">https://clb.ucl.cas.cz</a>';
					echo '</td></tr>';
