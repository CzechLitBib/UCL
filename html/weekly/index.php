...
<nav class="navbar container-fluid navbar-expand-md navbar-dark" style="background-color:#dc3545;">
	<div class="row align-items-center gx-0">
		...
		<div class="col"><a class="navbar-brand nav-link active" href="/main/">Vývoj # Týdenní kontrola</a></div>
	</div>
...
if (!empty($_SESSION['weekly'])){
	if (preg_match('/\d{4}-\d{2}-\d{2}/', $_SESSION['weekly'])) {
		...
		if (file_exists($file)) {
			...
			foreach($csv as $row) {
				echo '<tr><th class="text-center"><a class="external-link" target="_blank" href="https://aleph22.lib.cas.cz/F/?func=direct&doc_number=' . $row[0] . '&local_base=AV&format=001"><b>' . $row[0] . '</b></a></th>';
