	...
	if args.check:
		...
		if '072' in metadata:
			if len(metadata['072'].get_subfields('x')) != 1:
				write_error(header.identifier(), '072', SIF, '016', 'Chybí podpole 072x.')
		...
		if '041' in metadata:
			if metadata['041'].indicator1 + metadata['041'].indicator2 not in ('1 ', '0 '):
				write_error(header.identifier(), '041', SIF, '029', 'Chybný indikátor v poli 041.')
		if '072' in metadata:
			if metadata['072'].indicator1 + metadata['072'].indicator2 != ' 7':
				write_error(header.identifier(), '072', SIF, '030', 'Chybný indikátor v poli 072.')
		if '100' in metadata:
			if metadata['100'].indicator1 + metadata['100'].indicator2 not in ('3 ', '1 ', '0 '):
				write_error(header.identifier(), '100', SIF, '031', 'Chybný indikátor v poli 100.')
		if '110' in metadata:
			if metadata['110'].indicator1 + metadata['110'].indicator2 not in ('1 ', '2 '):
				write_error(header.identifier(), '110', SIF, '032', 'Chybný indikátor v poli 110.')
		if '245' in metadata:
			if metadata['245'].indicator1 not in ('0', '1'):
				write_error(header.identifier(), '245', SIF, '033', 'Chybný indikátor v poli 245.')
		if '520' in metadata:
		...
		if metadata.leader[7] == 'm':
			if '260' not in metadata and '264' not in metadata:
				write_error(header.identifier(), '260/264', SIF, '047', 'Chybný typ záznamu (pole 260/264).')
		for TAG in ('250', '490', '830'):
			if TAG in metadata:
				if metadata.leader[7] != 'm':
					write_error(header.identifier(), TAG, SIF, '048', 'Chybný typ záznamu (pole ' + TAG + ').')
		if metadata.leader[7] in ('a', 'b'):
			if '773' not in metadata:
				write_error(header.identifier(), '773', SIF, '049', 'Chybný typ záznamu (pole 773).')
		...
		if '100' in metadata:
			for SUB in metadata['100'].subfields[0::2]:
				if SUB not in ('a', 'b', 'c', 'd', 'g', '4', '7', 'x', 'q', 'j'):
					write_error(header.identifier(), '100', SIF, '052', 'Chybný kód podpole v poli 100.')
		if '110' in metadata:
			for SUB in metadata['110'].subfields[0::2]:
				if SUB not in ('a', 'b', 'c', 'd', 'n', '4', '7', 'x'):
					write_error(header.identifier(), '110', SIF, '053', 'Chybný kód podpole v poli 110.')
		if '111' in metadata:
			for SUB in metadata['111'].subfields[0::2]:
				if SUB not in ('a', 'b', 'c', 'd', 'n', '4', '7', 'x'):
					write_error(header.identifier(), '111', SIF, '054', 'Chybný kód podpole v poli 111.')
		if '245' in metadata:
			...
			for SUB in metadata['041'].subfields[0::2]:
				if SUB not in ('a', 'b', 'k', 'h'):
					if len(metadata['041'].get_subfields(SUB)) > 1:
						write_error(header.identifier(), '041', SIF, '081', 'Neplatné opakování podpolí v poli 041.')
		if '044' in metadata:
			for SUB in metadata['044'].subfields[0::2]:
				if SUB != 'a':
					if len(metadata['044'].get_subfields(SUB)) > 1:
						write_error(header.identifier(), '044', SIF, '082', 'Neplatné opakování podpolí v poli 044.')
		if '245' in metadata:
			for SUB in metadata['245'].subfields[0::2]:
				if SUB not in ('n', 'p'):
					if len(metadata['245'].get_subfields(SUB)) > 1:
						write_error(header.identifier(), '245', SIF, '083', 'Neplatné opakování podpolí v poli 245.')
		if '260' in metadata:
			for SUB in metadata['260'].subfields[0::2]:
				if SUB not in ('a', 'b'):
					if len(metadata['260'].get_subfields(SUB)) > 1:
						write_error(header.identifier(), '260', SIF, '084', 'Neplatné opakování podpolí v poli 260.')
		if '264' in metadata:
			for SUB in metadata['264'].subfields[0::2]:
				if SUB not in ('a', 'b'):
					if len(metadata['264'].get_subfields(SUB)) > 1:
						write_error(header.identifier(), '264', SIF, '085', 'Neplatné opakování podpolí v poli 264.')
		if '773' in metadata:
			for SUB in metadata['773'].subfields[0::2]:
				if SUB  != 'z':
					if len(metadata['773'].get_subfields(SUB)) > 1:
						write_error(header.identifier(), '773', SIF, '086', 'Neplatné opakování podpolí v poli 773.')
		if '787' in metadata:
			for SUB in metadata['787'].subfields[0::2]:
				if SUB not in ('k', 'z'):
					if len(metadata['787'].get_subfields(SUB)) > 1:
						write_error(header.identifier(), '787', SIF, '087', 'Neplatné opakování podpolí v poli 787.')
		...
		if '041' in metadata:
			if metadata['041'].indicator1 == '0':
				if 'h' in metadata['041']:
					write_error(header.identifier(), '041', SIF, '088', "Chybné podpole 'h' v poli 041.")
		if '041' in metadata:
			...
			if metadata['100'].indicator1 == '0':
				if 'a' in metadata['100']:
					if re.match('^.+\..+,.+$', metadata['100']['a']):
						write_error(header.identifier(), '100', SIF, '090', "Chybný znak v podpoli 'a' v poli 100.")
			if metadata['100'].indicator1 == '1':
				if 'c' in metadata['100']:
					if re.match('^\[.*$', metadata['100']['c']):
						write_error(header.identifier(), '100', SIF, '091', "Chybný znak v podpoli 'c' v poli 100.")
				#if 'a' in metadata['100']:
				#	if re.match('^.*\..*$', metadata['100']['a']):
				#		write_error(header.identifier(), '100', SIF, '092', "Chybný znak v podpoli 'a' v poli 100.")
		if '245' in metadata:
			if metadata['245'].indicator1 == '1':
				...
				if N != 1:
					write_error(header.identifier(), '245', SIF, '093', 'Neplatné pole, chybný indikátor v poli 245.')
				...
				if 'a' in metadata['245']:
					for S in ('The ', 'An ', 'Der ', 'Die ', 'Das ', 'Le ', 'La '):
						if re.match('^' + S + '.*', metadata['245']['a']):
							write_error(header.identifier(), '245', SIF, '095', 'Neplatný prefix, chybný 2.indikátor v poli 245.')
		if '600' in metadata:
			if metadata['600'].indicator1 == '0':
				if 'a' in metadata['600']:
					if re.match('^.*,.+$', metadata['600']['a']):
						write_error(header.identifier(), '600', SIF, '096', "Chybný znak v podpoli 'a' v poli 600.")
		for TAG in ('600', '610', '611', '630' ,'648', '650', '651', '655'):
			if TAG in metadata:
				if metadata[TAG].indicator2 == '7':
					if '2' not in metadata[TAG] or '7' not in metadata[TAG]:
						write_error(header.identifier(), TAG, SIF, '097', 'Chybný 2.indikátor v poli ' + TAG + '.')
		...
		if '008' in metadata:
			...
			if metadata['008'].value()[6] not in ('s', 'e', 'm', 'q'):
				write_error(header.identifier(), '008', SIF, '101', 'Chybný kód data v poli 008.')
			if not re.match('^\d+$', DATE) or len(DATE) not in (4, 6, 8):
					write_error(header.identifier(), '008', SIF, '102', 'Neplatné datum v poli 008.')
			if metadata['008'].value()[6] in ('s', 'q'):
				if len(DATE) != 4:
					write_error(header.identifier(), '008', SIF, '103', 'Nesoulad mezi kódem data a datem (má být RRRR).')
			if metadata['008'].value()[6] == 'e':
				if len(DATE) not in (6, 8):
					write_error(header.identifier(), '008', SIF, '104', 'Nesoulad mezi kódem data a datem (má být RRRRMM/RRRRMMDD).')
			if metadata['008'].value()[6] == 'm':
				if len(DATE) != 8:
					write_error(header.identifier(), '008', SIF, '105', 'Nesoulad mezi kódem data a datem (není interval roků).')
		if metadata.leader[7] == 'm':
			if '008' in metadata:
				...
				if '260' in metadata:
					if 'c' in metadata['260']:
						if DATA != metadata['260']['c'].strip('.'):
							write_error(header.identifier(), '008', SIF, '106', 'Nesoulad mezi daty v poli 008 a 260/264.')
				if '264' in metadata:
				...
				if '773' in metadata:
					if '9' in metadata['773']:
						...
						if DATA1[0:4] != DATA2 and DATA1[4:8] != DATA2 and DATA1 != DATA2:
							write_error(header.identifier(), '008', SIF, '108', 'Nesoulad mezi daty v poli 008 a 773-9.')
		if '008' in metadata:
			DATA = metadata['008'].value()[15:18].strip()
			if country_code:
				if DATA not in country_code:
					write_error(header.identifier(), '008', SIF, '109', 'Chybný kód země v poli 008.')
				if '044' in metadata:
					if DATA not in metadata['044'].subfields[1::2]:
						write_error(header.identifier(), '008', SIF, '110', 'Nesoulad mezi kódy zemí v poli 008 a 044.')
		if '008' in metadata:
			DATA = metadata['008'].value()[35:38].strip()
			if lang_code:
				if DATA not in lang_code:
					write_error(header.identifier(), '008', SIF, '111', 'Chybný kód jazyka v poli 008.')
				if '041' in metadata:
					if DATA not in metadata['041'].subfields[1::2]:
						write_error(header.identifier(), '008', SIF, '112', 'Nesoulad mezi kódy jazyků v poli 008 a 041.')
		if '041' in metadata:
			if lang_code:
				for DATA in metadata['041'].subfields[1::2]:
					if DATA not in lang_code:
						write_error(header.identifier(), '041', SIF, '113', 'Chybný kód jazyka v poli 041.')
			if len(metadata['041'].subfields) != 4:
					write_error(header.identifier(), '041', SIF, '114', 'Pole 041 musí obsahovat 2 a více podpolí.')
		if '044' in metadata:
			if country_code:
				for DATA in metadata['044'].subfields[1::2]:
					if DATA not in country_code:
						write_error(header.identifier(), '044', SIF, '115', 'Chybný kód země v poli 044.')
			if len(metadata['044'].subfields) != 4:
					write_error(header.identifier(), '044', SIF, '116', 'Pole 044 musí obsahovat 2 a více podpolí.')
		if '100' in metadata:
			if role_code:
				if '4' in metadata['100']:
					for DATA in metadata['100'].get_subfields('4'):
						if DATA not in role_code:
							write_error(header.identifier(), '100', SIF, '117', 'Chybný kód role v poli 100.')
			if 'j' in metadata['100']:
				if metadata['100']['j'] not in ('bbg', 'rej'):
					write_error(header.identifier(), '100', SIF, '118', 'Chybný kód role v podpoli 100j.')
				if '4' not in metadata['100'] or 'oth' not in metadata['100'].get_subfields('4'):
						write_error(header.identifier(), '100', SIF, '119', 'V poli 100 chybí podpole 4 s hodnotou "oth".')
		if '700' in metadata:
			if role_code:
				if '4' in metadata['700']:
					for DATA in metadata['700'].get_subfields('4'):
						if DATA not in role_code:
							write_error(header.identifier(), '700', SIF, '120', 'Chybný kód role v poli 700.')
			if 'j' in metadata['700']:
				if metadata['700']['j'] not in ('bbg', 'rej'):
					write_error(header.identifier(), '700', SIF, '121', 'Chybný kód role v podpoli 700j.')
				if '4' not in metadata['700'] or 'oth' not in metadata['700'].get_subfields('4'):
						write_error(header.identifier(), '700', SIF, '122', 'V poli 700 chybí podpole 4 s hodnotou "oth".')
		if '100' in metadata:
			if '4' in metadata['100']:
				DATA = metadata['100'].get_subfields('4')
				if 'aut' not in DATA and 'ive' not in DATA:
					write_error(header.identifier(), '100', SIF, '123', 'V poli 100 chybí kód role "aut" nebo "ive".')
